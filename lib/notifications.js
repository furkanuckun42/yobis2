...
export async function sendSystemNotification({ userId, message, tab }) {
  try {
    ...
    // 4. Web Push Bildirimi Gönderimi (Cihaz Bildirimleri - Sadece Adminler)
    try {
...
export async function notifyAdmins({ message, tab }) {
  try {
    ...
    const promises = admins.map(admin =>
      sendSystemNotification({
        ...
        message,
        tab
      }).catch(err => console.error(`Admin notification failed for user ${admin.id}:`, err))
    )
