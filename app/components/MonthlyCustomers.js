    ...
    <div className="space-y-8 animate-fade-in">
      ...
      <div className="flex flex-row flex-nowrap bg-violet-950/30 p-1 rounded-xl border border-violet-500/10 w-full md:w-fit overflow-x-auto gap-1">
        <button
          ...
          className={`flex-shrink-0 px-2.5 py-2 sm:px-4 sm:py-2.5 rounded-lg text-[10px] sm:text-xs font-bold uppercase tracking-wider transition whitespace-nowrap cursor-pointer flex items-center justify-center gap-1.5 ${
            activeTabFilter === 'active'
              ? 'bg-violet-600 text-white glow-purple shadow-sm'
              : 'text-gray-400 hover:text-white'
          }`}
        >
          <Clock className="w-3.5 h-3.5 flex-shrink-0 hidden sm:inline-block" />
          <span>
            <span className="inline sm:hidden">Devam Eden ({activeCards.length})</span>
            <span className="hidden sm:inline">Devam Edenler ({activeCards.length})</span>
          </span>
        </button>
        <button
        ...
        >
          <CreditCard className="w-3.5 h-3.5 flex-shrink-0 hidden sm:inline-block" />
          <span>
            <span className="inline sm:hidden">Ödeme Bekleyen ({completedCards.length})</span>
            <span className="hidden sm:inline">Ödeme Bekleyenler ({completedCards.length})</span>
          </span>
        </button>
        <button
        ...
        >
          <CheckCircle2 className="w-3.5 h-3.5 flex-shrink-0 hidden sm:inline-block" />
          <span>
            <span className="inline sm:hidden">Ödenen ({archivedCards.length})</span>
            <span className="hidden sm:inline">Ödenenler & Arşiv ({archivedCards.length})</span>
          </span>
        </button>
      </div>
      ...
      <div key={card.id} className="p-5 rounded-2xl glass border border-violet-500/10 hover:border-violet-500/20 transition-all flex flex-col justify-between space-y-4 shadow-lg group relative">
        ...
        <div className="space-y-2">
          ...
          <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
            {card.items.length === 0 ? (
              ...
              card.items.map(item => (
                <div key={item.id} className="flex p-2.5 rounded-xl bg-violet-950/20 border border-violet-500/5 hover:border-violet-500/10 hover:bg-violet-950/30 transition-all duration-200 gap-2.5 items-start">
                  {/* Left Column: Checkbox */}
                  <button
                    disabled={isArchived}
                    onClick={() => handleToggleItem(item, !item.completed)}
                    className={`w-4 h-4 flex items-center justify-center rounded border transition cursor-pointer flex-shrink-0 mt-0.5 ${
                      item.completed
                  ...
                  <div className="flex-1 min-w-0 flex flex-col gap-1.5">
                    {/* Row 1: Title (or editor) and Actions */}
                    ...
                    {editingItemId === item.id ? (
                      <div className="flex flex-wrap items-center gap-2 pt-1.5 border-t border-violet-500/5 mt-0.5">
                        {/* Inline Assignee Selection */}
                        <div className="relative flex items-center bg-[#0c061a] border border-violet-500/10 rounded-lg hover:border-violet-500/30 transition-all h-6">
                          <User className="absolute left-1.5 w-2.5 h-2.5 text-violet-400 pointer-events-none" />
                          <select
                            value={item.task?.assignedUserId || ''}
                            onChange={(e) => handleUpdateItemAssignee(item.id, e.target.value)}
                            className="bg-transparent border-none text-[10px] text-violet-300 focus:outline-none cursor-pointer font-bold pl-5 pr-5 py-0 min-w-[75px] h-full appearance-none"
                            title="Atanan Kişi"
                          >
                          ...
                          <ChevronDown className="absolute right-1.5 w-2.5 h-2.5 text-violet-400 pointer-events-none" />
                        </div>

                        {/* Inline Due Date Picker */}
                        <div className="relative flex items-center bg-[#0c061a] border border-violet-500/10 rounded-lg hover:border-violet-500/30 transition-all h-6">
                          <Calendar className="absolute left-1.5 w-2.5 h-2.5 text-violet-400 pointer-events-none" />
                          <input
                            type="date"
                            ...
                            className="bg-transparent border-none text-[10px] text-violet-300 w-[95px] focus:outline-none cursor-pointer font-bold pl-5 py-0 h-full"
                            title="Teslim Tarihi"
                          />
                        </div>
                      </div>
                      ...
                      (item.task?.assignedUser || item.task?.dueDate) && (
                        <div className="flex flex-wrap items-center gap-1.5 pt-1 border-t border-violet-500/5 mt-0.5 text-gray-500">
        ...
        {!isArchived && (
          <div className="border-t border-violet-500/10 pt-3.5 space-y-2">
            ...
            <div className="flex items-center gap-2">
              {/* Assignee Selection */}
              <div className="relative flex-1 flex items-center bg-violet-950/15 border border-violet-500/5 rounded-lg text-gray-400 focus-within:border-violet-500/20 transition cursor-pointer h-9">
                ...
                <select
                  ...
                  className="w-full bg-transparent border-none text-[10px] text-gray-300 focus:outline-none cursor-pointer pl-8 pr-8 py-1.5 appearance-none h-full"
                >
              ...
              <div className="relative flex-1 flex items-center bg-violet-950/15 border border-violet-500/5 rounded-lg text-gray-400 focus-within:border-violet-500/20 transition cursor-pointer h-9">
                ...
                <input
                  ...
                  className="w-full bg-transparent border-none text-[10px] text-gray-300 focus:outline-none cursor-pointer pl-8 py-1.5 h-full"
                />
              </div>
            </div>
          </div>
